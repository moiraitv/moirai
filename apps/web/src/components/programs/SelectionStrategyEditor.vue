...
import { Dices, ListOrdered, Shuffle } from '@lucide/vue';

/** Selection strategies currently supported by content programs. */
type SelectionStrategy = 'sequential' | 'shuffle' | 'random';
...
<template>
	<section class="program-editor-section">
		...
		<div class="strategy-card-grid">
			<button v-for="strategy in [{ id: 'sequential', label: 'Sequential', detail: 'Play in order, continuing where you left off', icon: ListOrdered }, { id: 'shuffle', label: 'Shuffle (no repeats)', detail: 'Random order with no repeats until all have played', icon: Shuffle }, { id: 'random', label: 'Random', detail: 'Choose a random item each time', icon: Dices }] as const" :key="strategy.id" type="button" :class="{ active: modelValue === strategy.id }" :aria-pressed="modelValue === strategy.id" @click="emit('update:modelValue', strategy.id)">
				<component :is="strategy.icon" :size="29" /><strong>{{ strategy.label }}</strong><span>{{ strategy.detail }}</span>
			</button>
			<button type="button" disabled title="Weighted selection is planned for later"><span class="strategy-star">☆</span><strong>Weighted random</strong><span>Coming later</span></button>
		</div>
