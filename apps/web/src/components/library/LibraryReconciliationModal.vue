...
<template>
	<div class="modal-backdrop" @click.self="emit('close')">
		<section class="modal reconciliation-modal">
			...
			<p v-if="reconciliation.status === 'observing-removals'">Ordinary removals are applied after {{ reconciliation.requiredObservations }} complete observations at least {{ reconciliation.observationIntervalMinutes }} minutes apart.</p>
			<div v-if="reconciliation.missingItems.length" class="reconciliation-items"><article v-for="item in reconciliation.missingItems" :key="item.id"><div><strong>{{ item.title }}</strong><small>{{ item.relativePath }}</small></div><span>{{ item.consecutiveObservations }}/{{ reconciliation.requiredObservations }}</span></article><small v-if="reconciliation.pendingRemovalCount > reconciliation.missingItems.length">Showing the first {{ reconciliation.missingItems.length }} of {{ reconciliation.pendingRemovalCount }} missing items.</small></div>
