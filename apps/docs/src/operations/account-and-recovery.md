...
The command prints a single-use recovery link that expires after 15 minutes. Open it in a trusted browser, choose new local credentials, and discard the link. The password is entered in the browser, not on the command line.

Changing the Logto endpoint or application identifier revokes existing provider-backed sessions the next time Moirai starts.
