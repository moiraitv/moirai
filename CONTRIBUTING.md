...
Preview the guide with `npm run docs:user:dev` using the URL it prints. Unreviewed pages show their topic ID and repository-relative source path at the top; `/help/review.html` lists outstanding reviews. `npm run docs:user:review:list -- --json` provides a machine-readable queue.
